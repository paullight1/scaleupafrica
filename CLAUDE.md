...
ScaleUp Africa — a directory and funding-intelligence platform for African SMEs. Users create public business profiles, browse a searchable directory, and (with an active subscription) get AI-curated funding opportunities. The project was generated with Lovable (lovable.dev); changes pushed to the repo sync back to the Lovable project.
...
`AuthProvider` in `src/hooks/useAuth.tsx` wraps the router and exposes `{ user, session, loading, signOut }` via `useAuth()`. OAuth goes through `src/integrations/lovable/index.ts` (Lovable cloud auth → `supabase.auth.setSession`).

### Generated files — do not hand-edit

- `src/integrations/supabase/client.ts` and `src/integrations/lovable/index.ts` are auto-generated by Lovable.
