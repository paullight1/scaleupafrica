#!/usr/bin/env bash
# Regenerate the branding raster assets in public/ from the SVG sources in scripts/.
#
...
#   npx playwright screenshot --viewport-size=1200,630 scripts/og-banner.html public/og-banner.png
#
...
sips -s format png scripts/og-banner.svg --out public/og-banner.png >/dev/null
...
im = Image.open('public/og-banner.png').convert('RGB')
im.quantize(colors=256, method=Image.MEDIANCUT, dither=Image.FLOYDSTEINBERG).save('public/og-banner.png', optimize=True)
PY

echo "favicon.svg (copy source)"
cp scripts/favicon.svg public/favicon.svg

echo "apple-touch-icon.png (180x180)"
sips -s format png scripts/apple-touch-icon.svg --out public/apple-touch-icon.png >/dev/null

echo "icon-192.png / icon-512.png"
sips -s format png scripts/favicon.svg --resampleHeightWidth 192 192 --out public/icon-192.png >/dev/null
sips -s format png scripts/favicon.svg --resampleHeightWidth 512 512 --out public/icon-512.png >/dev/null
...
node scripts/png-to-ico.mjs public/favicon.ico /tmp/_fav32.png /tmp/_fav48.png
