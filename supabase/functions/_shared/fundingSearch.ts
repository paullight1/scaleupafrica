...
export function fundingSearchReasons(
  ...
  if (countryMatches.length > 0) {
    reasons.push(`Available in ${countryMatches.slice(0, 2).join(", ")}.`);
  }
  if (tagMatches.length > 0) {
    reasons.push(`Matches ${tagMatches.slice(0, 3).join(", ")} focus.`);
  }
  if (titleMatches.length > 0) {
    reasons.push(`Program title matches ${titleMatches.slice(0, 3).join(", ")}.`);
  }
  if (funderMatches.length > 0) {
    reasons.push(`Matches the ${opportunity.funder} funder.`);
  }
...
/** Verified items are passed first and therefore always win collisions. */
