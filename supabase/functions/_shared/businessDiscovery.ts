...
interface EvidencePage { url: string; contentType: string; text: string; }
interface AiCandidate {
  canonical_name?: unknown; website?: unknown; country?: unknown; summary?: unknown;
  organisation_type?: unknown; sectors?: unknown; operating_countries?: unknown;
  founding_year?: unknown; keywords?: unknown; source_urls?: unknown; field_evidence?: unknown;
}
...
export async function discoverBusinessCandidates(input: {
  ...
  for (const url of urls) {
    ...
    evidence.push({ url: result.url, contentType: result.contentType, text: compactEvidence(result.body) });
  }
  if (!evidence.length) return { ok: false, error: "no_evidence" };

  const aiKey = input.aiApiKey?.trim() ?? "";
  if (!aiKey) return { ok: false, error: "provider_unavailable" };

  const extracted = await extractCandidatesFromEvidence(input.businessName, input.countryHint, evidence, aiKey);
  if (!extracted.ok) return extracted;

  const allowedEvidenceUrls = new Set(evidence.map((page) => page.url));
  const evidenceByUrl = new Map(evidence.map((page) => [page.url, page.text] as const));
  const candidates = extracted.candidates
    .map((candidate) => normalizeExtractedCandidate(candidate, allowedEvidenceUrls, evidenceByUrl))
...
async function searchBrave(
  ...
  try { payload = await response.json() as BraveResponse; } catch { return { ok: false, error: "provider_error" }; }
  const results: SearchCandidate[] = [];
  for (const raw of payload.web?.results ?? []) {
    const candidateUrl = validHttpUrl(raw.url);
    if (!candidateUrl) continue;
    results.push({ title: safeString(raw.title, 240), url: candidateUrl, description: safeString(raw.description, 600) });
...
function normalizeExtractedCandidate(
  ...
  evidenceByUrl: ReadonlyMap<string, string>,
  ...
  const fieldEvidence = isRecord(raw.field_evidence)
    ? sanitizeEvidenceMap(raw.field_evidence, sourceUrls, evidenceByUrl)
    : {};
  const canonicalName = hasFieldEvidence(fieldEvidence, "canonical_name") ? safeString(raw.canonical_name, 200) : "";
  ...
  const organisationType = hasFieldEvidence(fieldEvidence, "organisation_type") ? nullableString(raw.organisation_type, 80) : null;
  const sectors = hasFieldEvidence(fieldEvidence, "sectors") ? stringArray(raw.sectors, 12, 80) : [];
  const operatingCountries = hasFieldEvidence(fieldEvidence, "operating_countries") ? stringArray(raw.operating_countries, 30, 120) : [];
  ...
  return {
    id: crypto.randomUUID(), canonicalName, website, country, summary, sourceUrls,
    enrichedProfile: {
...
function sanitizeEvidenceMap(
  ...
  for (const [key, raw] of Object.entries(value).slice(0, 30)) {
    if (!/^[a-z0-9_]{1,80}$/i.test(key) || !Array.isArray(raw)) continue;
    const proofs: Array<{ url: string; quote: string }> = [];
    for (const item of raw.slice(0, 10)) {
      if (!isRecord(item)) continue;
      const evidenceUrl = validHttpUrl(item.url);
      const quote = safeString(item.quote, 400);
      if (!evidenceUrl || !allowed.has(evidenceUrl) || !quote) continue;
      const pageText = evidenceByUrl.get(evidenceUrl);
      if (!pageText || !pageText.includes(quote)) continue;
      proofs.push({ url: evidenceUrl, quote });
    }
    if (proofs.length) out[key] = proofs;
  }
  return out;
}
...
function validHttpUrl(raw: unknown): string | null {
  ...
  try { const url = new URL(raw.trim()); if (url.protocol !== "http:" && url.protocol !== "https:") return null; url.hash = ""; return url.href; } catch { return null; }
}
function safeString(raw: unknown, max: number): string { return typeof raw === "string" ? raw.trim().slice(0, max) : ""; }
function nullableString(raw: unknown, max: number): string | null { const value = safeString(raw, max); return value || null; }
function stringArray(raw: unknown, maxItems: number, maxLength = 1000): string[] {
  if (!Array.isArray(raw)) return [];
  return Array.from(new Set(raw.map((value) => safeString(value, maxLength)).filter(Boolean))).slice(0, maxItems);
}
...
function dedupeUrls(values: string[]): string[] {
  ...
  for (const raw of values) { const url = validHttpUrl(raw); if (!url || seen.has(url)) continue; seen.add(url); out.push(url); }
  return out;
}
