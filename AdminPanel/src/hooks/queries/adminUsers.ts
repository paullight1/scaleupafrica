...
export function useSetRole() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // Role writes go through the admin_set_role RPC, not the table. The
      // browser client only holds SELECT on user_roles (migration
      // 20260720120000 line 53), so a direct insert/delete fails with 42501
      // before RLS is ever consulted. The RPC is SECURITY DEFINER and
      // re-checks is_admin(auth.uid()) itself.
