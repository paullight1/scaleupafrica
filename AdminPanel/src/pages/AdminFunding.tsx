...
const FORM_DEFAULTS: FundingFormValues = {
  title: "",
  funder: "",
  type: "",
  summary: "",
...
function rowToForm(row: FundingRow): FundingFormValues {
  return {
    title: row.title ?? "",
    funder: row.funder ?? "",
    type: row.type ?? "",
    ...
    url: row.url ?? "",
    tags: (row.tags ?? []).join(", "),
    country_focus: (row.country_focus ?? []).join(", "),
    status: (row.status as FundingFormValues["status"]) ?? "draft",
    featured: !!row.featured,
  };
}

function formToPayload(values: FundingFormValues): FundingFormPayload {
  return {
...
const AdminFunding = () => {
  ...
  const onSubmit = handleSubmit((values) => {
    save.mutate(
      { id: editing?.id, values: formToPayload(values) },
  ...
  const aiDraftCount = useMemo(
    () => rows.filter((r) => r.source === "ai" && r.status === "draft").length,
    [rows],
  );
  ...
  return (
    <>
      ...
      <PageHeader
        title="Funding"
        subtitle="Curate and verify the funding opportunities shown to members."
        actions={
      ...
      {aiDraftCount > 0 && (
        <div className="mt-6 flex items-center gap-3 rounded-xl border border-border bg-surface-muted p-4 text-sm">
          <Bot className="h-5 w-5 shrink-0 text-primary-dark" aria-hidden="true" />
          <p className="text-ink-strong">
            <span className="font-semibold">{aiDraftCount}</span> AI-sourced{" "}
            {aiDraftCount === 1 ? "draft is" : "drafts are"} awaiting review. Open one to review and
            publish.
          </p>
        </div>
      )}
      ...
      <div className="mt-6">
        {query.isLoading ? (
          ...
          <EmptyState
            variant={hasFilters ? "search" : "firstRun"}
            title="No opportunities found"
            description={
              hasFilters
                ? "Try adjusting your filters."
                : "Add your first funding opportunity to show it to members."
            }
            action={hasFilters ? undefined : { label: "New opportunity", onClick: openCreate }}
          />
        ) : (
                  ...
                  return (
                    <TableRow key={row.id}>
                      <TableCell className="max-w-xs">
                        <div className="font-medium text-ink-strong">{row.title}</div>
                        <div className="text-sm text-muted-foreground">{row.funder}</div>
                        {isAiDraft && (
                      ...
                      <TableCell>
                        {row.featured ? (
                          <Star className="h-4 w-4 fill-primary text-primary" aria-label="Featured" />
                        ) : (
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {row.status === "published" ? (
                                <DropdownMenuItem onClick={() => setStatus.mutate({ row, status: "draft" })}>
                                  <EyeOff className="h-4 w-4" /> Unpublish
                                </DropdownMenuItem>
                              ) : (
                                <DropdownMenuItem
                                  onClick={() => setStatus.mutate({ row, status: "published" })}
                                >
                                  <Eye className="h-4 w-4" /> Publish
                                </DropdownMenuItem>
                              )}
                              <DropdownMenuItem
                                onClick={() => toggleFeatured.mutate({ row, featured: !row.featured })}
                              >
                                <Star className="h-4 w-4" />
                                {row.featured ? "Unfeature" : "Feature"}
                              </DropdownMenuItem>
                              <DropdownMenuItem
                                onClick={() => user && verify.mutate({ row, verifiedBy: user.id })}
                              >
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {editing
                ? "Update the details, then save. Publish to show it to members."
                : "Add a curated funding opportunity."}
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="f-funder">Funder *</Label>
                <Input id="f-funder" {...register("funder")} />
                {errors.funder && (
                  <p className="text-sm text-destructive-strong">{errors.funder.message}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea id="f-summary" rows={3} {...register("summary")} />
            </div>

            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="f-type">Type</Label>
                <Input id="f-type" placeholder="Grant, Accelerator…" {...register("type")} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="f-amount">Amount</Label>
                <Input id="f-amount" placeholder="Up to $50,000" {...register("amount")} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="f-opens">Opens</Label>
                <Input id="f-opens" {...register("opens")} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="f-deadline">Deadline</Label>
                <Input id="f-deadline" {...register("deadline")} />
              </div>
            </div>

            <div className="space-y-1.5">
              <Label htmlFor="f-eligibility">Eligibility</Label>
              <Textarea id="f-eligibility" rows={2} {...register("eligibility")} />
            </div>

            <div className="space-y-1.5">
              <Label htmlFor="f-url">URL</Label>
              <Input id="f-url" type="url" placeholder="https://…" {...register("url")} />
              {errors.url && <p className="text-sm text-destructive-strong">{errors.url.message}</p>}
            </div>

            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="f-tags">Tags (comma-separated)</Label>
                <Input id="f-tags" placeholder="women, fintech" {...register("tags")} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="f-countries">Country focus (comma-separated)</Label>
                <Input id="f-countries" placeholder="Nigeria, Kenya" {...register("country_focus")} />
              </div>
            </div>
            ...
            <div className="grid items-end gap-4 sm:grid-cols-2">
              ...
              <div className="flex items-center justify-between rounded-lg border border-border p-3">
                <Label htmlFor="f-featured">Featured</Label>
                <Controller
                  control={control}
                  name="featured"
                  render={({ field }) => (
                    <Switch id="f-featured" checked={field.value} onCheckedChange={field.onChange} />
                  )}
                />
              </div>
            </div>
