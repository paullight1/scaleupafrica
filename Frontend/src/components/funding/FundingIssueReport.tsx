...
export function FundingIssueReport({ opportunityId }: { opportunityId: string }) {
  ...
  if (submitted) {
    return <p className="mt-2 text-xs text-muted-foreground">Thanks — this opportunity is queued for source review.</p>;
  }

  const submit = async () => {
    setSubmitting(true);
    try {
      const { error } = await supabase.from("funding_opportunity_reports").insert({
    ...
    <div className="mt-2 border-t border-border/70 pt-2">
      {!open ? (
        ...
        <div className="space-y-3 rounded-lg bg-surface-subtle p-3">
          ...
          <select
            id={`funding-report-category-${opportunityId}`}
            value={category}
          ...
          <textarea
            id={`funding-report-message-${opportunityId}`}
            value={message}
            onChange={(event) => setMessage(event.target.value.slice(0, 1000))}
            rows={3}
            className="w-full rounded-md border border-border bg-background p-3 text-sm"
            placeholder="What did you see on the official source?"
          />
