...
/**
 * The profile editor, in two modes over one form.
 *
 *   No profile yet  → a three-step wizard (identity → story → matching).
 *   Profile exists  → section editing, scoped by `?section=`.
 *
 * Both modes share `SECTION_FIELDS`, one schema and one submit path, so a rule
 * can't hold in one and not the other. This replaces `/directory/create`, a
 * single fifteen-field wall that a brand-new user met before they had any reason
 * to trust the product, and that returning users could only re-enter through
 * fragile `#anchor` links from the dashboard.
 */
export function DashboardProfileEdit() {
  ...
  // First-run pre-fill. Signup step 3 stores the founder's name and business
  // name on user_metadata; seeding them here means a new user meets a form that
  // already knows who they are. Metadata is user-controlled, so it is only ever
  // a default — the schema still validates it, and an existing profile row
  // always wins (the effect above owns that case).
  useEffect(() => {
    ...
    // Leave `hydrated` unset when there is nothing to seed, so a profile row
    // arriving later still gets to hydrate the form properly.
  ...
  const setImage =
    (field: "logo_url" | "founder_photo_url") =>
    (url: string) => {
  ...
  const onSubmit = async (values: ProfileFormValues) => {
    ...
    try {
      ...
      // useSaveProfile invalidates the DIRECTORY caches; the dashboard reads the
      // same row under its own key, so without this the page we're about to
      // navigate to would render the values the user just changed away from.
      await qc.invalidateQueries({ queryKey: qk.myProfile(user.id) });

      // Best-effort orphan cleanup — only AFTER a successful save. Failures are ignored.
      ...
      reset(values); // clears isDirty so the unsaved-changes guard lets us leave
  ...
  // ---------------------------------------------------------------------------
  // Wizard (first-time) vs section editing (returning)
  // ---------------------------------------------------------------------------
  ...
  /** Sections rendered right now. Wizard: one. Edit: the requested one, or all. */
  const visibleSections: ProfileSection[] = wizard
    ? [step]
    : requested
      ? [requested]
      : [...PROFILE_SECTIONS];
  ...
  async function nextStep() {
    // Validate only this step's fields — a wizard that rejects step 1 because
    // step 3 is empty is just a long form with extra clicks.
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <Link
          to={DASHBOARD_PROFILE}
          className="inline-flex items-center gap-1 text-sm font-medium text-muted-foreground hover:text-foreground"
        >
          <ArrowLeft className="h-4 w-4" aria-hidden="true" />
          {isEditing ? "Back to my profile" : "Not now"}
        </Link>

        <h1 className="mt-3 font-display text-2xl font-semibold text-ink-strong md:text-3xl">
          {wizard ? "Create your profile" : SECTION_META[visibleSections[0]].title}
        </h1>
        ...
        {wizard ? (
          <div className="mt-3 flex items-center gap-3">
            <div
              ...
              aria-label={`Step ${stepIndex + 1} of ${WIZARD_STEPS.length}`}
            >
              <div
          ...
          <p className="mt-1 text-muted-foreground">
            {requested
              ? SECTION_META[requested].blurb
              : "Everything on your public profile, in one place."}
          </p>
        )}
      </div>
          ...
          <form onSubmit={handleSubmit(onSubmit, onInvalid)} noValidate className="space-y-8">
            {visibleSections.map((section) => (
              ...
              >
                ...
                <ProfileSectionFields section={section} userId={user.id} />
              </section>
            ))}

            {/* Sticky action bar — on a long form the save button must not be a
                scroll away from the field that needs saving. */}
            <div className="sticky bottom-0 -mx-4 border-t border-border bg-background/95 px-4 py-4 backdrop-blur md:-mx-6 md:px-6">
              <div className="flex flex-wrap items-center gap-3">
                  ...
                  <Button type="submit" size="lg" disabled={save.isPending}>
                    {save.isPending ? (
                      ...
                      <>
                        ...
                        {isEditing ? "Save changes" : "Publish my profile"}
                      </>
                    )}
                  </Button>
                )}
                ...
                {!wizard && (
                  <Button type="button" variant="ghost" size="lg" onClick={requestCancel}>
                    Cancel
                  </Button>
                )}

                {isDirty && !save.isPending && (
                  <span className="ml-auto text-sm text-muted-foreground">Unsaved changes</span>
                )}
              </div>
            </div>
          </form>
