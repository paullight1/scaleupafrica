...
export function recommendOpportunity<T extends RecommendationOpportunity>(
  ...
  const storedApplicationStatus = opportunity.applicationStatus ?? "unknown";
  const applicationStatusFresh = isStatusFresh(storedApplicationStatus, opportunity.statusCheckedAt, now);
  const applicationStatus = effectiveFundingStatus(storedApplicationStatus, opportunity.statusCheckedAt, now);
  const sourceVerified = opportunity.verificationStatus === "verified";
  ...
  const matchScore = possible.value > 0
    ...
    : 0;
  const primaryApplyEligible =
    sourceVerified &&
    applicationStatusFresh &&
    eligibilityResult.eligibilityStatus === "eligible" &&
    (applicationStatus === "open" || applicationStatus === "closing_soon" || applicationStatus === "rolling");
