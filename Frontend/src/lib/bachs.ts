...
export async function initCheckout(params: {
  ...
  const { data, error } = await supabase.functions.invoke<InitResponse>("bachs-init", {
    body: params,
  });
  if (error) {
    ...
    async ({
      ...
      try {
        ...
        if (result.code === "ALREADY_ACTIVE") {
          ...
          toast.error("Checkout is temporarily busy. Please try again shortly.");
        } else if (result.code === "NOT_CONFIGURED") {
          toast.error("Online payment is temporarily unavailable. Please contact support.");
        } else {
