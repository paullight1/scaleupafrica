  ...
  if (!access) {
    return (
      <main className="min-h-screen bg-secondary flex items-center justify-center px-6 py-24">
        <div className="max-w-xl w-full rounded-xl border border-border bg-card p-10 shadow-elevated">
          <div className="mx-auto mb-6 h-16 w-16 rounded-xl bg-navy text-white flex items-center justify-center">
            <Lock className="h-8 w-8" />
          </div>
          <h1 className="font-display text-2xl font-bold text-foreground mb-3 text-center">Members only</h1>
          ...
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-5 mb-5 space-y-3">
            <div className="flex items-start gap-3">
              <AlertTriangle className="h-5 w-5 text-primary-dark shrink-0 mt-0.5" />
              <div className="text-sm text-foreground/80 space-y-2">
                ...
                <p className="flex items-start gap-2">
                  <ShieldAlert className="h-4 w-4 text-primary-dark shrink-0 mt-0.5" />
                  <span>
            ...
            <label className="flex items-start gap-3 cursor-pointer pt-3 border-t border-primary/30">
              <Checkbox
          ...
          <div className="text-center">
            {acknowledged ? (
              <Link to="/#pricing"><Button variant="default" size="lg">See membership</Button></Link>
            ) : (
              <Button variant="default" size="lg" disabled>See membership</Button>
            )}
          </div>
    ...
    <main className="min-h-screen bg-background">
      <header className="bg-navy text-white py-16 px-6">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="rounded-xl border border-primary/30 bg-white/5 p-4 mb-6 text-xs text-white/80 flex items-start gap-2">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            />
            <Button variant="default" size="lg" onClick={generate} disabled={fetching}>
      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        ...
        <div className="grid gap-6">
          {opps.map((o, i) => {
            ...
            return (
              <article key={i} className="rounded-xl border border-border bg-card p-6 shadow-soft hover:border-primary/40 transition-all">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      {o.type && <span className="rounded-full bg-navy/10 px-2.5 py-0.5 text-xs font-semibold text-navy">{o.type}</span>}
                      {o.travel_component && <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-semibold text-primary-dark"><Plane className="h-3 w-3" /> Travel</span>}
                    </div>
                    <h3 className="font-display text-xl font-bold text-foreground mb-1">{o.title}</h3>
                ...
                {isOpen && (
                  <div className="mt-6 space-y-5 border-t border-border pt-6">
                    {o.funder_about && (
                      <div>
                        <h4 className="flex items-center gap-2 text-sm font-semibold text-navy mb-2"><Info className="h-4 w-4" /> About the funder</h4>
                        <p className="text-sm text-foreground/80">{o.funder_about}</p>
                      </div>
                    )}
                    {o.travel_component && (
                      <div>
                        <h4 className="flex items-center gap-2 text-sm font-semibold text-navy mb-2"><Plane className="h-4 w-4" /> Travel / exchange component</h4>
                        <p className="text-sm text-foreground/80">{o.travel_component}</p>
                      </div>
                    )}
                    {o.sdg_focus && o.sdg_focus.length > 0 && (
                      <div>
                        <h4 className="flex items-center gap-2 text-sm font-semibold text-navy mb-2"><Target className="h-4 w-4" /> SDG focus</h4>
                        <div className="flex flex-wrap gap-2">
                          {o.sdg_focus.map((s) => <span key={s} className="rounded-full bg-navy/10 px-2.5 py-0.5 text-xs text-navy">{s}</span>)}
                        </div>
                      </div>
                    )}
                    {o.past_recipients && o.past_recipients.length > 0 && (
                      <div>
                        <h4 className="flex items-center gap-2 text-sm font-semibold text-navy mb-2"><Users className="h-4 w-4" /> Businesses previously funded</h4>
                        <ul className="space-y-2">
                          {o.past_recipients.map((r, idx) => (
                            <li key={idx} className="text-sm text-foreground/80">
                              ...
                              {r.website && <> · <a href={r.website} target="_blank" rel="noopener noreferrer" className="text-navy hover:text-navy-light underline-offset-4 hover:underline">website</a></>}
                    ...
                    {o.application_tips && o.application_tips.length > 0 && (
                      <div>
                        <h4 className="flex items-center gap-2 text-sm font-semibold text-navy mb-2"><Lightbulb className="h-4 w-4" /> Tips for a stellar application</h4>
                    ...
                    {o.important_notes && (
                      <div>
                        <h4 className="flex items-center gap-2 text-sm font-semibold text-navy mb-2"><Info className="h-4 w-4" /> Important to note</h4>
                        <p className="text-sm text-foreground/80">{o.important_notes}</p>
                      </div>
                    )}

                    <div className="rounded-lg bg-primary/10 p-3 text-xs text-foreground/80 flex items-start gap-2">
                      <ShieldAlert className="h-4 w-4 text-primary-dark shrink-0 mt-0.5" />
